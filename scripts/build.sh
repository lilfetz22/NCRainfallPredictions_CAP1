...
check_prereqs() {
	...
	if [ -f "$DIRNAME/config/.admin.secret" ]; then
		if [ -z "$(egrep '^admin:[A-Za-z0-9 @#$%^&*()~.,:;_+=<>?-]+$' "$DIRNAME/config/.admin.secret")" ]; then
			((missing_prereqs++)); echo >&2 "PREREQ ERROR: .admin.secret file must match syntax 'admin:password' (no quotes).";
		fi
	else
		((missing_prereqs++)); echo >&2 "MISSING PREREQ: $(basename "$DIRNAME")/config/.admin.secret file missing.";
	fi
